# Jira Sync — SCRUM Project

> Auto-synced from [omnizya.atlassian.net](https://omnizya.atlassian.net/jira/software/projects/SCRUM/boards/1) on 2026-03-24.
> Project: **Anaq io Scrum** (key: SCRUM) | 98 issues total (93 original + 5 created)

---

## Changes Made (2026-03-24)

**Duplicates marked (8):** SCRUM-10, 12, 13, 14, 15, 16, 34, 47 — summaries prefixed with `[DUPLICATE of SCRUM-XX]`

**New issues created (5):**
| Key | Summary | Assignee | Parent | Due | Label |
|-----|---------|----------|--------|-----|-------|
| SCRUM-95 | Expo Kiosk Mode — 1920x1080 landscape fullscreen demo (Rock 3) | Moughamir | SCRUM-73 | Mar 28 | expo, Q1-Rock |
| SCRUM-96 | Studio MVP — End-to-end generation flow polish (Rock 1) | Moughamir | SCRUM-73 | Mar 28 | expo, Q1-Rock |
| SCRUM-97 | 100 waitlist signups pre-expo — tracking & push (Rock 4) | Amal | SCRUM-76 | Mar 28 | expo, Q1-Rock |
| SCRUM-98 | Brand assets finalized — logo, business cards, pitch deck (Rock 5) | Amal | — | Mar 28 | expo, Q1-Rock |
| SCRUM-99 | Expo demo dry run — validate full flow on target hardware | Moughamir | SCRUM-73 | Mar 27 | expo, Q1-Rock |

**COMPANY-OS.md updated:** Rocks table now cross-references Jira issue keys. Tech debt section updated with Jira hygiene items.
...
## Backoffice Sprint (Active)

The current development sprint focuses on the **backoffice admin dashboard**:

```
SCRUM-61 [In Progress] — Admin/Manager Dashboard
├── SCRUM-68 [In Progress] — Admin auth middleware (role-based route guard)
├── SCRUM-69 [To Do] — Waitlist table UI (sortable, filterable, inline status)
├── SCRUM-70 [To Do] — Funnel chart + KPI summary cards UI
└── SCRUM-71 [To Do] — CSV export (filtered waitlist for CRM / email)

SCRUM-62 [In Progress] — User State Management
├── SCRUM-63 [In Progress] — Add `status` field to users DB schema + migration
├── SCRUM-64 [To Do] — Backend API — state transition service with role guard
├── SCRUM-65 [To Do] — Audit log — record all state changes
├── SCRUM-66 [To Do] — Brevo webhook — auto-update to `communicated` on delivery
└── SCRUM-67 [To Do] — Email notifications — trigger on approved / rejected

SCRUM-26 [To Do] — Authentication & User Account System (Yasser)
└── SCRUM-94 [To Do] — Create Supabase database for Backoffice/Dashboard
```
...
1. ~~**Expo deadline is 4 days away (Mar 28)** but Jira doesn't reflect the expo Rocks~~ — **FIXED:** Created SCRUM-95/96/97/98/99 for all 5 Rocks with due dates and labels.
2. ~~**Significant duplicate issues**~~ — **FIXED:** 8 duplicates marked with `[DUPLICATE]` prefix (SCRUM-10, 12, 13, 14, 15, 16, 34, 47). Transition to Done or delete at your discretion.
3. **79 of 98 issues are "To Do"** — backlog is large relative to team capacity (2 founders + 2 contributors). Consider archiving post-Q2 items.
4. **Many items unassigned** — especially in the newer epics (SCRUM-42 through 46).
5. **No sprint structure visible** — issues aren't grouped into time-boxed sprints. Consider creating a "Expo Sprint" board filter.
6. **The "omnizya" assignee** appears to be the Jira admin account — should be resolved to Moughamir or Amal on 12+ issues.
