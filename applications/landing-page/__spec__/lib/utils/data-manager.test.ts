...
import { DataManager, DataManagerWithExtras, type TranslationFn } from '@/lib/utils/data-manager'
...
describe('DataManager', () => {
  it('returns raw translation data for a given key', () => {
    ...
    const result = DataManager<Array<{ num: string; title: string }>>(t, 'steps')
...
describe('DataManagerWithExtras', () => {
  it('merges extras onto each raw item', () => {
    ...
    const result = DataManagerWithExtras(t, 'audiences', icons, (item, icon) => ({ ...item, icon }))
  ...
  it('passes the index to the merge function', () => {
    ...
    const result = DataManagerWithExtras(t, 'items', colors, (item, color, index) => ({
      ...item,
      color,
      index,
    }))
