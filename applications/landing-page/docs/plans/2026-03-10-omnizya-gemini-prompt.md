...
// Split "ANAQIO" into chars, animate each
{
  'ANAQIO'.split('').map((char, i) => (
    <motion.span
      ...
      className="font-display inline-block font-light"
...
```tsx
{
  ProblemSectionText.painPoints.map((point, i) => (
    <motion.div
      ...
      className="border-aq-blue/30 flex items-start gap-4 border-l pl-4"
      style={{
        position: 'absolute',
        top: `${48 + i * 13}%`,
        left: '8.33%',
...
<motion.h2
  ...
  className="font-display text-foreground absolute left-[8.33%] top-[18%] z-20 max-w-[14ch] font-light leading-[0.95]"
>
  Fashion moves fast.
  <br />
...
>
  ...
  <motion.span
    ...
    className="font-display text-aq-blue text-4xl font-light"
  >
    {step.num}
  </motion.span>
  ...
...
<div role="tablist" className="border-border flex gap-8 border-b">
  {audiences.map((a, i) => (
    <button
      ...
      onClick={() => setActive(i)}
      className="font-label tracking-label text-muted-foreground aria-selected:text-foreground relative pb-3 text-xs uppercase transition-colors"
    >
      {a.label}
      ...
      {active === i && (
        <motion.div
          ...
          className="bg-aq-blue absolute bottom-0 left-0 h-px w-full"
...
<AnimatePresence mode="wait">
  ...
  >
    ...
    <p
      data-atom
      className="font-body text-muted-foreground mt-4 max-w-[40ch] text-sm leading-relaxed"
    >
      {audience.body}
    </p>
...
return words.map((word, i) => {
  ...
  return (
    <motion.span
      ...
      className="font-display text-foreground mr-[0.28em] inline-block text-[clamp(1.6rem,3vw,3rem)] font-light leading-tight"
    >
      {word}
    </motion.span>
  );
...
<span
  ...
  data-decorative
  aria-hidden="true"
  className="font-display text-foreground/[0.03] pointer-events-none absolute inset-x-0 top-[5%] z-0 select-none text-center font-light"
...
>
  ...
  <div
    ...
    data-decorative
    aria-hidden="true"
    className="bg-aq-blue/10 pointer-events-none absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full blur-[100px]"
  />
  ...
  <motion.p
    data-atom
    {...fadeIn(reduced, 0)}
    className="font-label tracking-label text-muted-foreground absolute top-[20%] text-xs uppercase"
  >
...
<input
  className="border-border font-display text-foreground placeholder:text-muted-foreground/30 focus:border-aq-blue w-full border-0 border-b bg-transparent pb-3 text-[clamp(1.4rem,2.5vw,2.2rem)] transition-colors focus:outline-none"
...
>
  <AnaqioLogo className="text-aq-blue mx-auto mb-6 h-8 w-auto" />
