...
test.describe('Locale Switcher — Component Visibility', () => {
  ...
  test('shows current locale flag for each starting locale', async ({ page }) => {
...
test.describe('Locale Switcher — Dropdown Interaction', () => {
  ...
  test('dropdown options show full language names', async ({ page }) => {
    ...
    await expect(page.getByTestId('locale-option-en-US')).toContainText('English')
    await expect(page.getByTestId('locale-option-fr-FR')).toContainText('Français')
    await expect(page.getByTestId('locale-option-ar-MA')).toContainText('العربية')
  })
...
test.describe('Locale Switcher — URL Navigation', () => {
  ...
  test('switching back to English updates URL to /en-US/*', async ({ page }) => {
...
test.describe('Locale Switcher — RTL Support', () => {
  ...
  test('English locale has no RTL direction on html element', async ({ page }) => {
...
test.describe('Locale Switcher — Active State', () => {
  test('active locale option has aria-selected=true', async ({ page }) => {
    ...
    await expect(enOption.locator('xpath=..')).toHaveAttribute('aria-selected', 'true')
  })

  test('inactive locale options do not have aria-selected', async ({ page }) => {
    await page.goto('/en-US')
    await page.getByTestId('locale-switcher-trigger').click()

    const frParent = page.getByTestId('locale-option-fr-FR').locator('xpath=..')
    const arParent = page.getByTestId('locale-option-ar-MA').locator('xpath=..')

    await expect(frParent).not.toHaveAttribute('aria-selected', 'true')
    await expect(arParent).not.toHaveAttribute('aria-selected', 'true')
  })

  test('trigger shows short code of active locale after switching', async ({ page }) => {
    await page.goto('/en-US')
    await page.getByTestId('locale-switcher-trigger').click()
    await page.getByTestId('locale-option-fr-FR').click()
