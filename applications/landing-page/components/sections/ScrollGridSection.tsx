...
export function ScrollGridSection() {
  ...
  const scalerScaleX = useTransform(scrollYProgress, [0, 0.5], [vw / naturalW, 1])
  const scalerScaleY = useTransform(scrollYProgress, [0, 0.5], [vh / naturalH, 1])
  const scalerRadius = useTransform(scrollYProgress, [0.35, 0.5], ['0px', '8px'])
  const scalerOpacity = useTransform(scrollYProgress, [0, 0.3, 0.6, 1], [1, 1, 0.85, 0.85])
    ...
    <section ref={sectionRef} className="relative h-[280vh]" aria-hidden="true">
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden bg-[#2b3ae6]/40">
        <div className="scroll-grid">
          <GridLayer items={LAYER_1} layerIndex={0} progress={scrollYProgress} />

          <GridLayer items={LAYER_2} layerIndex={1} progress={scrollYProgress} />

          <GridLayer items={LAYER_3} layerIndex={2} progress={scrollYProgress} />
